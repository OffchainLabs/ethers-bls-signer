...
export function newKeyPairFromHexKey(privateKey: string): keyPair {
    const secret = frFromHexKey(privateKey);
    const mclPubkey = mcl.mul(g2(), secret);
    mclPubkey.normalize();
    const pubkey = g2ToHex(mclPubkey);
    return { pubkey, secret };
...
export function frFromHexKey(r: string): mclFR {
    let fr = new mcl.Fr();
    // TODO: should we set string instead of hash?
    // https://github.com/herumi/mcl-wasm/blob/d26de1b005965ac82b3eac607d0b5d376119f98b/mcl.js#L349-L351
    // or https://github.com/herumi/mcl-wasm/blob/d26de1b005965ac82b3eac607d0b5d376119f98b/mcl.js#L378-L380
    fr.setHashOf(r);
    return fr;
}
